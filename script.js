...
window.onload = function () {
    ...
    // Event listeners — dentro do onload para garantir que os elementos existem
    const dateInput = document.getElementById('reservation-date');
    if (dateInput) {
        dateInput.addEventListener('change', () => { updateVisuals(); fetchData(); });
    }

    const closeBtn = document.getElementById('reservation-modal');
    if (closeBtn) {
        closeBtn.querySelector('.close').addEventListener('click', closeModal);
    }
...
function setupDateRestrictions() {
    ...
    if (rangeText) rangeText.textContent = `Período permitido: ${start.toLocaleDateString('pt-BR')} a ${end.toLocaleDateString('pt-BR')}`;
}
...
function generateSeats() {
    document.querySelectorAll('.row').forEach(row => {
        ...
        for (let f = 1; f <= cfg.fileiras; f++) {
            ...
            const s = (f - 1) * cfg.assentosPorBaia + 1;
            const e = f * cfg.assentosPorBaia;
            for (let i = s; i <= e; i++) {
                const seat = document.createElement('div');
                seat.className = 'seat available';
                seat.textContent = i;
                seat.dataset.id  = `${loc}-${rNum}-${i}`;
                seat.dataset.loc = loc;
                seat.dataset.row = rNum;
                seat.dataset.num = i;
...
function selectSeat(seat) {
    ...
    departmentRules.forEach(r => { sel.innerHTML += `<option value="${r.area}">${r.area}</option>`; });

    document.getElementById('selected-seat').textContent =
        `${seat.dataset.loc} - Fileira ${seat.dataset.row} - Posição ${seat.dataset.num}`;
...
function openCheckinList() {
    ...
    if (todayRes.length === 0) {
        ...
        const areas = {};
        todayRes.forEach(r => { if (!areas[r.local]) areas[r.local] = []; areas[r.local].push(r); });
        let html = '';
...
function selectCheckin(encodedRes) {
    ...
    stBtn.textContent = '🚀 Iniciar Teste'; stBtn.disabled = false;

    const submitBtn = document.getElementById('checkin-submit-btn');
    submitBtn.textContent = '✅ Confirmar Check-in'; submitBtn.disabled = false;
...
function measureUploadXHR(url, sizeBytes) {
    return new Promise((resolve, reject) => {
        ...
        xhr.upload.onprogress = (e) => {
            ...
            if (firstProgressAt === null) { firstProgressAt = now; lastLoaded = e.loaded; lastTime = now; return; }
        ...
        xhr.upload.onloadend = () => {
            if (samples.length >= 1) {
                ...
                resolve(((top.reduce((a, b) => a + b) / top.length) * 8 / 1e6).toFixed(2));
            ...
            } else { reject(new Error('Sem dados de upload')); }
        };
...
async function runCheckinSpeedTest() {
    ...
    } catch (e) { console.warn('Erro download:', e); dlMbps = 'Erro'; }

    btn.textContent = '⏳ Testando upload...';
    try {
        const STREAMS = 4, SIZE = 8000000, ts = Date.now();
        const start = performance.now();
        await Promise.all(
            Array.from({ length: STREAMS }, (_, i) => {
                const fd = new FormData();
        ...
        try { ulMbps = await measureUploadXHR(API_URL, 3000000); }
        catch (e2) { ulMbps = 'Erro'; }
    }
...
async function submitCheckin() {
    ...
    keys.forEach(key => {
        ...
        params.append(key + '_defeito', val === 'Vermelho' ? document.getElementById('defeito_' + key).value.trim() : '');
    });
    ...
    try {
        ...
        if (json.success) {
            ...
            alert('❌ ' + (json.message || json.error || 'Tente novamente.'));
            btn.textContent = '✅ Confirmar Check-in'; btn.disabled = false;
        }
    } catch (err) {
